...
public class CritereSearch extends SearchBase implements Serializable {
	...
	public SearchResultBean tranform(List<Object> resultQuery) {
		...
		CritereVOTransformer CritereVOTransformer = new CritereVOTransformer();
		for (Object o : resultQuery) {
			Critere opp = (Critere) o;
			listParamSearchResultsVo
					.add((CritereVO) CritereVOTransformer
							.model2VO(opp));
		}
	...
	public List specificExecute(Serializable searchBean,
		...
		CritereCriteria criteria = (CritereCriteria) searchBean;
		criteria.setSearchConfig(config);
		criteria.setSortField(sortField);
		criteria.setSortOrder(sortOrder);
		ReferentielService referentielService = (ReferentielService) BeansLookuper
				.lookup("referentielService");
		return referentielService.findCritereByCriteria(criteria);
	}
