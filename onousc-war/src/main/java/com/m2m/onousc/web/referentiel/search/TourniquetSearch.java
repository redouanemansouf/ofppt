...
public class TourniquetSearch extends SearchBase implements Serializable {
	...
	public SearchResultBean tranform(List<Object> resultQuery) {
		...
		List<ISearchResultVO> listTourniquetSearchResultsVo = new ArrayList<ISearchResultVO>();
		TourniquetVOTransformer TourniquetVOTransformer = new TourniquetVOTransformer();
		for (Object o : resultQuery) {
			Tourniquet Tourniquet = (Tourniquet) o;
			listTourniquetSearchResultsVo
					.add((TourniquetVO) TourniquetVOTransformer
							.model2VO(Tourniquet));
		}
		resultBean.setRows(listTourniquetSearchResultsVo);
	...
	public List specificExecute(Serializable searchBean, SearchConfig config,
		...
		TourniquetCriteria criteria = (TourniquetCriteria) searchBean;
		criteria.setSearchConfig(config);
		criteria.setSortField(sortField);
		criteria.setSortOrder(sortOrder);
		ReferentielService referentielService = (ReferentielService) BeansLookuper
				.lookup("referentielService");
		return referentielService.findTourniquetByCriteria(criteria);
	}
