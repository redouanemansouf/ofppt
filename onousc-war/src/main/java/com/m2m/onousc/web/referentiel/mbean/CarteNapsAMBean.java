...
@ManagedBean
@ViewScoped
public class CarteNapsAMBean extends BaseMBean {
	...
	private static final ILogger LOGGER = SL4JLogger.getInstance(CarteNapsAMBean.class);
...
//		webBean.setListSiteForCombo(referentielSrvPresentation.findAllSite());

		// File directory = neaw File("C:\\FichiersMX");
...
//				 new FacesMessage(FacesMessage.SEVERITY_INFO,
//				 "Le fichier à été importé : \n" + "\t \t \t \t \t \t \t \t"
//						 +" \n Nombre de lignes importées :	"+imp+" \n Nombre de lignes rejetées :	" +rejet, "Import Done"));
...
//			impo.setId(webBean.getVo().getId());
	...
	protected boolean isAutorise() {
		return hasEcran(Constantes.CODE_GESTION_ENROL_M);
	}
