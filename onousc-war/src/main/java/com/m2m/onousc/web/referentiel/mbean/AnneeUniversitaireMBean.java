...
@ManagedBean
@ViewScoped
public class AnneeUniversitaireMBean extends BaseMBean {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1790772238459700696L;
	private AnneeUniversitaireWebBean webBean;
	ReferentielSrvPresentation referentielService = (ReferentielSrvPresentation) BeansLookuper
			.lookup("referentielSrvPresentation");

	public void init() {
		webBean = new AnneeUniversitaireWebBean();
	}

	public void resetWebBeanVO() {
		webBean.setVo(new AnneeUniversitaireVO());
	}

	public void resetWebBeanSelectedVO() {
		webBean.setSelectedVOs(new ArrayList<AnneeUniversitaireVO>());
	}
	...
	public void creer() {
		...
		error = delegate.creerAnneeUniversitaire(webBean);
		if (!error) {
	...
	public void modifier() {
		clearReport();
		ReferentielBusDelegate delegate = new ReferentielBusDelegate();
		error = delegate.modifierAnneeUniversitaire(webBean);
		if (!error) {
	...
	public void activer() {
		...
		if (!webBean.getSelectedVOs().isEmpty()) {
			ReferentielBusDelegate delegate = new ReferentielBusDelegate();
			error = delegate.activerAnneeUniversitaire(webBean);
			if (error) {
	...
	public void desactiver() {
		...
		if (!webBean.getSelectedVOs().isEmpty()) {
			ReferentielBusDelegate delegate = new ReferentielBusDelegate();
			error = delegate.desactiverAnneeUniversitaire(webBean);
			if (error) {
				JSFUtils.contextExecute("_errorDialog()");
			} else {
	...
	public void supprimer() {
		clearReport();
		ReferentielBusDelegate delegate = new ReferentielBusDelegate();
		error = delegate.supprimerAnneeUniversitaire(webBean);
		if (error) {
			JSFUtils.contextExecute("_errorDialog()");
		} else {
	...
	public AnneeUniversitaireWebBean getWebBean() {
		return webBean;
	}

	public void setWebBean(AnneeUniversitaireWebBean webBean) {
