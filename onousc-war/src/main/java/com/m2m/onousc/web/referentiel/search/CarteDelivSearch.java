 ...
 * @author msadik
 */
public class CarteDelivSearch extends SearchBase implements Serializable {
	...
	public SearchResultBean tranform(List<Object> resultQuery) {
		...
		List<ISearchResultVO> listyDelivSearchResultsVo = new ArrayList<ISearchResultVO>();
		DelivranceVOTransformer delivranceVOTransformer = new DelivranceVOTransformer();
		for (Object o : resultQuery) {
			DelivranceCarte deliv = (DelivranceCarte) o;
			listyDelivSearchResultsVo.add((DelivranceVO) delivranceVOTransformer.model2VO(deliv));
		}
		resultBean.setRows(listyDelivSearchResultsVo);
	...
	public List specificExecute(Serializable searchBean, SearchConfig config, String sortField, String sortOrder) {
		DelivranceCriteria criteria = (DelivranceCriteria) searchBean;
		...
		return referentielService.findDelivByCriteria(criteria);
	}
}
