 ...
 * @author msadik
 */
@FacesConverter("detailProfilConverter")
public class DetailProfilConverter implements Converter {
	...
	public Object getAsObject(FacesContext context, UIComponent component,
		...
		if (value != null && value.trim().length() > 0) {
			try {
				...
				return servicePres.findDetailProfil(Long.parseLong(value));
			} catch (NumberFormatException e) {
				throw new ConverterException(new FacesMessage(
						FacesMessage.SEVERITY_ERROR, "Erreur de conversion",
						"Veuillez choisir un canal de vente"));
			}
		} else {
			return null;
		}
	...
	public String getAsString(FacesContext context, UIComponent component,
			Object value) {
		if (value != null) {
			return String.valueOf(((DetailProfilVO) value).getId());
		} else {
			return null;
		}
	}
