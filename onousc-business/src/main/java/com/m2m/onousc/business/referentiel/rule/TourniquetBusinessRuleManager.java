...
public class TourniquetBusinessRuleManager extends BusinessRuleManagerBase{
	...
	public ProcessResult doValidation() {
		...
		if(sousCommand.equals("creerTourniquet")){
			processResult = rule1();
	...
	private ProcessResult rule1(){
		ProcessResult processResult = new ProcessResult();
		Tourniquet model = (Tourniquet) subject;
		List<Tourniquet> listcaisse =  referentielService.findTourniquetByCode(model.getCode());
